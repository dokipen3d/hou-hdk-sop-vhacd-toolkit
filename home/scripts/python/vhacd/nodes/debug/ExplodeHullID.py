'''
    ...
    explodeHullID = toolutils.createModuleFromSection('explodehullid', kwargs['type'], 'ExplodeHullID.py')  
    
    - requires line below to be placed in callback of Description toggle 
    hou.phm().explodeHullID.Callback(kwargs)
'''
...
def SetDefaultState(node):
    ...
    node.parm('explodehullidpower').revertToDefaults()
